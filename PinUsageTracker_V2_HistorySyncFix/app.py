...
summary_rows = get_summary_for_date(str(selected_date))
...
weighted_yield = (
    (summary_df["Yield %"] * summary_df["Expected Pins"]).sum() / total_expected
    if total_expected > 0
    else 0
)
...
col1.metric("Kettles Planned", f"{total_kettles:,.2f}".rstrip("0").rstrip("."))
col2.metric("Expected Pins", f"{total_expected:,.2f}".rstrip("0").rstrip("."))
col3.metric("Yield", f"{weighted_yield:.2f}%")
col4.metric("New Expected", f"{total_new_expected:,.2f}".rstrip("0").rstrip("."))
col5.metric(
    "Actual Pins",
    f"{total_actual:,.2f}".rstrip("0").rstrip("."),
    f"{total_actual - total_new_expected:+,.2f}".rstrip("0").rstrip("."),
)
...
st.dataframe(
    ...
    hide_index=True,
    column_config={
        "Kettle Planned": st.column_config.NumberColumn(format="%.2f"),
        "Expected Pins": st.column_config.NumberColumn(format="%.2f"),
        "Yield %": st.column_config.NumberColumn(format="%.2f%%"),
        "New Expected": st.column_config.NumberColumn(format="%.2f"),
        "Actuals": st.column_config.NumberColumn(format="%.2f"),
...
st.download_button(
    ...
    data=summary_df.to_csv(index=False, float_format="%.2f").encode("utf-8"),
